                            ...
                            <template #header>
                                <div class="card-header">
                                    <h1 class="card-header-title"
                                        @click="toUpdatePage(catalog.articleId, catalog)">
                                        {{ catalog.title }}
                                    </h1>
                                    <span>
                                            <span>
...
//===========================================================================
//更新按钮
function toUpdatePage(articleId, catalog) {
    router.push("/update/" + articleId);
    //将被选中的文章目录缓存至sessionStorage以便在Update.vue中获取该值
    const catalogString = JSON.stringify(catalog);
    window.sessionStorage.setItem("catalog", catalogString);
}
//放入回收站按钮（标记删除)
function dropArticle(articleId) {
    getRequest("/admin/article/drop?articleId=" + articleId).then(res =>{
...
//获取文章目录的总数
const total = ref(0);
getRequest('/admin/article/total').then(res => {
...
//获取第一页文章目录
getRequest('/admin/article/catalog?pageIndex=1').then(res => {
    if (res.data.code === 200) {
        catalogs.value = res.data.content;
        //缓存当前页
        const catalogsString = JSON.stringify(res.data.content);
        window.sessionStorage.setItem("managePage_index_1", catalogsString);
    } else {
        ElMessage.error(res.data.message);
    }
});
