...
//获取摘要总数（摘要总数=公开的文章总数）
const total = ref(0);
getRequest('/article/totalPublic').then(res => {
...
const changePage = (pageIndex: number) => {
    //尝试从缓存中获取数据
    const cache = window.sessionStorage.getItem("homepage_index_" + pageIndex);
    //检查当前页是否有缓存
    if (cache) {
        //有缓存则从缓存获取
        ...
        //没缓存则从后台获取
        getRequest("/article/summary?pageIndex=" + pageIndex).then(res => {
            if (res.data.code === 200) {
                //更新数据
                ...
                window.sessionStorage.setItem("homepage_index_" + pageIndex, cachedSummaries);
            } else {
                ElMessage.error("未能从后台服务器获取到数据");
            }
        })
