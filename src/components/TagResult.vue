...
postRequest("/tag/summaryWithTag/" + tagName + "/of/1", null).then(res => {
    if (res.data.code === 200) {
        ...
        window.sessionStorage.setItem("tag_result_of_" + tagName + "_index_1", cachedSummaries);
    } else {
...
const changePage = (pageIndex: number) => {
    const prefix = "tag_result_of_" + tagName + "_index_";
    ...
    if (cache) {
        ...
        //没缓存则从后台获取
        postRequest("/tag/summaryWithTag/" + tagName + "/of/" + pageIndex, null).then(res => {
            if (res.data.code === 200) {
                //更新数据
                summaries.value = res.data.content;
                //回到页顶
